...
verify_app_writable() {
    directory=$1
    if ! /opt/base/sbin/su-exec app sh -c 'probe="$1/.gdownloader-write-test-$$"; : > "$probe" && rm -f "$probe"' sh "$directory"; then
...
/opt/base/sbin/su-exec app sh -c '. /usr/local/lib/gdownloader/bootstrap.sh; prepare_state /config /output /defaults/gdownloader/config.json'
